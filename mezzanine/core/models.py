...
from mezzanine.core.managers import DisplayableManager, CurrentSiteManager, CurrentLanguageManager
...
class LanguageRelated(models.Model):
    ...
    objects = CurrentLanguageManager()

    class Meta:
        abstract = True

    language = models.CharField(max_length=5, default='', editable=False)
    ...
    def save(self, *args, **kwargs):
        ...
        to ``True``.
        """
        if self.language is None or self.language=='':
            self.language = get_language()
        super(LanguageRelated, self).save(*args, **kwargs)
...
class Displayable(LanguageRelated, Slugged, MetaData, TimeStamped):
    """
