...
class CurrentLanguageManager(Manager):
    def get_queryset(self):
        lookup = {"language__exact": get_language()}
        return super(CurrentLanguageManager, self).get_queryset().filter(**lookup)

class DisplayableManager(CurrentSiteManager, CurrentLanguageManager, 
                         PublishedManager, SearchableManager):
    """
